...
import {fetchMeasuredSensor,fetchStationInfoById,ai} from '../../../functions/apiActions'
...
class Dashboard_detail extends React.Component {
  ...
  else if(this.state.measured_data.length > 0 && this.state.measured_data.length <= max_display ){
  ...
  render() {
    ...
    const curent_value = this.state.measured_data[this.state.measured_data.length-1]
        ...
        <Row>
          <Col md={3} style={{background: 'aliceblue'}}>
            ...
            <Card className="m-2 shadow bg-white rounded-lg border-0" color="light">
              <CardBody>
                <p><Badge color="primary"> สถานะ : </Badge> {station_info.station_status}</p>
                <p><Badge color="light"> ชื่อสถานี : </Badge> {station_info.stationName}</p>
                <p><Badge color="light">  ประเภทสถานี : </Badge> {station_info.stationType}</p>
                <p><Badge color="light">  ตำแหน่ง :  </Badge>{station_info.latitude}</p>
                {curent_value ?
                <p><Badge color="light"> อัพเดทล่าสุด : </Badge>{curent_value.datetime}</p>
                  : 
                <p><Badge color="light"> อัพเดทล่าสุด:</Badge>{0}</p>
                }
                <p><Badge color="light"> userId :  </Badge>{station_info.userId}</p>
                <p><Badge color="light"> stationId :  </Badge>{station_info.stationId}</p>
                <p><Badge color="light"> Volt : </Badge>{' '} {station_info.power} {' '} DC</p>
              </CardBody>
            </Card>
            <Card className="m-2 shadow bg-white rounded-lg border-0" color="light">
              <CardBody>
                <CardText><Badge color="primary">การตั้งค่าปัจจุบัน</Badge></CardText>
                <CardText><Badge color="light"> เวลาเริ่มต้นการทำงาน : </Badge>{' '} {this.state.stations.start_time}</CardText>
                <CardText><Badge color="light"> เวลาสิ้นสุดการทำงาน : </Badge>{' '} {this.state.stations.end_time}</CardText>
                <CardText><Badge color="light"> ตรวจทุกๆ : </Badge>{' '}{this.state.stations.check_every_h} { ' ชม.'} {this.state.stations.check_every_m} {' นาที '}</CardText>
            ...
            <Row style={{justifyContent: 'space-evenly'}}>
              <Button color="danger" onClick={this.toggleDelete}> ลบสถานี <BsFillTrashFill/></Button>{' '} 
              <Button color="warning" onClick={this.toggleEdit}> แก้ไข <BsScrewdriver/></Button>
            </Row>
            <br></br>
            <Row style={{justifyContent: 'space-evenly'}}>
              <Button style={{width:'80%'}} color="primary" onClick={this.toggleControl}>ควบคุม</Button>{' '}
            </Row>
            <br></br>
            <Row style={{justifyContent: 'space-evenly'}}>
              <Button style={{width:'80%'}} color="primary" onClick={this.aiPred}>ทำนายคุณภาพน้ำด้วยระบบปัญญาประดิษฐ์</Button>{' '}
            </Row>
          ...
          {curent_value ?
            <Row>
              ...
              <Dashstatus dash_name = {'Temperature'} value = {curent_value.temperature} activate = {station_info.tempSensorStatus}> </Dashstatus>
              <Dashstatus dash_name = {'pH'} value = {curent_value.ph} activate = {station_info.phSensorStatus}></Dashstatus>
              <Dashstatus dash_name = {'Turbidity'} value = {curent_value.turbidity} activate = {station_info.turbiditySensorStatus}></Dashstatus>
              <Dashstatus dash_name = {'DO'} value = {curent_value.do} activate = {station_info.doSensorStatus}></Dashstatus>
              <Dashstatus dash_name = {'EC'} value = {curent_value.ec} activate = {station_info.ecSensorStatus}></Dashstatus>
              <Dashstatus dash_name = {'TDS'} value = {curent_value.tds} activate = {station_info.tdsSensorStatus}></Dashstatus>
              <Dashstatus dash_name = {'Salinity'} value = {curent_value.salinity} activate = {station_info.salSensorStatus}></Dashstatus>
              <Dashstatus dash_name = {'Ammonia'} value = {curent_value.ammonia} activate = {station_info.salSensorStatus}></Dashstatus>
              <GaugeChartdetail value={curent_value}/>
            </Row>
            :
            <Row>
              <Dashstatus dash_name = {'Temperature'} value = {0}> </Dashstatus>
